...
function Results (props) {
  ...
  <If condition={props.loading}>
    ...
    <Else>
      <When condition={ props.results && props.results.length > 0 }>
        <>
          <h3 data-testid="count">Count: {props.count}</h3>
          {/* <h3>Headers: {JSON.stringify(props.headers)}</h3> */}
          <div>Headers: <JSONPretty data={props.headers} theme={JSONPrettyMon} padding={4} ></JSONPretty></div>
          <div data-testid="results">Results: <JSONPretty data={props.results} theme={JSONPrettyMon} padding={4} ></JSONPretty></div>
        </>
      </When>
    </Else>
  </If>
